...
locals {
  ...
  default_layer_arn = "arn:${data.aws_partition.current.partition}:lambda:${data.aws_region.current.name}:${local.dd_account_id}:layer:Datadog-Forwarder:${local.layer_version}"
